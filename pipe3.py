...
from AptAvatar.wan.modules import (CLIPModel, T5EncoderModel, WanVAE)
from AptAvatar.infinite_talk.modules.multitalk_model import WanModel
from AptAvatar.infinite_talk.audio_analysis.wav2vec2 import Wav2Vec2Model
from AptAvatar.infinite_talk.utils.multitalk_utils import match_and_blend_colors_torch, resize_and_centercrop
...
def normalize_device_map_target(device, cpu_offload=False):
    if cpu_offload:
        return "cpu"
...
class TaoAvatarPipeline:
    def __init__(
        ...
        model_load_device = normalize_device_map_target(
            self.device,
            cpu_offload=self.cpu_offload,
        )

        # Use the safetensors/HuggingFace checkpoint under checkpoint_dir.
        ...
        # Disable compiled VAE under multi-GPU USP because it can corrupt
        # recurrent latent state across chunks during distributed inference.
        if COMPILE_VAE and not self.cpu_offload and not self.use_usp:
            self.vae.encode = torch.compile(self.vae.encode)
            self.vae.decode = torch.compile(self.vae.decode)
        elif COMPILE_VAE and self.use_usp and self.rank == 0:
            logger.info("Skip torch.compile for VAE in multi-GPU mode.")
    ...
    def _get_module_device(self, module, fallback=None):
        try:
            return next(module.parameters()).device
        except StopIteration:
            return fallback if fallback is not None else torch.device(self.device)
    ...
    def prepare_params(self,
        ...
        self.set_input_prompt(input_prompt)
    ...
    def generate(self, audio_embedding):
        ...
        model_device = self._get_module_device(self.model, fallback=torch.device(self.device))
        # evaluation mode
        with torch.no_grad():
            audio_embedding = self._move_to_device(audio_embedding, model_device)
            self.arg_c = self._move_to_device(self.arg_c, model_device)
            self.latent_motion_frames = self._move_to_device(self.latent_motion_frames, model_device)
            ...
            for i in range(len(self.timesteps)-1):
                ...
                latent = (1 - t_i_1) * x_0 + t_i_1 * torch.randn(x_0.size(), dtype=x_0.dtype, device=model_device, generator=self.generator)
